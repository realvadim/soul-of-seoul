...
<div id="content">
	<?php	
		...
		if(empty($login) or empty($password)) 
			errorGoBack("./login.php", "Вы ввели не всю информацию!",$values,"Попробовать снова");
		...
		if ($row['col'] > 2)
			errorGoBack("./about_seoul.php", "Вы набрали имя пользователя или пароль неверно 3 раза. Доступ к аккаунту заблокирован на 15 минут.",$values,"На главную");         
		...
		if (empty($row['id'])) {
			...
			errorGoBack("./login.php", "Введённое вами имя пользователя и/или пароль неверны!",$values,"Попробовать снова");
		}
		else {
